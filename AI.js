...
function aiLevel4() {
  ...
  // follow edge in clockwise (?) direction
  // goes down if trapped, but a SMARTER one would see if any obstacles are its tail
  if (followLeftEdge) {
    if (!(inSnake((snake[0][0] - 1) + '_' + snake[0][1])) || snake[0][0] < 1) {
      dir="left";
    } else if (!(inSnake(snake[0][0] + '_' + (snake[0][1] - 1))) || snake[0][0] < 1) {
      dir="up";
    } else if (!(inSnake((snake[0][0] + 1) + '_' + snake[0][1])) || snake[0][0] < 1) {
      dir="right";
    } else {
      dir="down";
    }
    monteCarlo(1, 1); // dummy values
    return moveHead(dir);
  }
  if (dir == 'right') {
    // Blocked by wall
